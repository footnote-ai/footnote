...
- The label mapping lives in `docs/agents/triage-labels.md`. For this repo the
  canonical names and the label strings are identical.
- Issue types come from the org types `Feature`, `Bug`, and `Task`; do not
  recreate types as labels. Labels do not carry priority, effort, or dates
  (those live in org issue fields), and the org Project holds workflow status.
- Read the shared glossary at `docs/ai/CONTEXT.md` and respect ADRs in
  `docs/ai/adr/` in the area you're triaging.
- Every comment or issue posted to the issue tracker during triage must start
  with the AI disclaimer shown above.
