...
enable_trustgraph_runtime() {
  local app_name="$1"
  echo "Enabling TrustGraph HTTP retrieval with required ownership validation for $app_name..."
  fly secrets set \
...
if [[ "$enable_trustgraph" == true ]]; then
  required_secrets+=(TAILSCALE_AUTHKEY EXECUTION_CONTRACT_TRUSTGRAPH_ADAPTER_API_TOKEN EXECUTION_CONTRACT_TRUSTGRAPH_BASE_URL EXECUTION_CONTRACT_TRUSTGRAPH_FLOW EXECUTION_CONTRACT_TRUSTGRAPH_COLLECTION EXECUTION_CONTRACT_TRUSTGRAPH_WORKSPACE_REF EXECUTION_CONTRACT_TRUSTGRAPH_OWNERSHIP_ENDPOINT_URL EXECUTION_CONTRACT_TRUSTGRAPH_OWNERSHIP_API_TOKEN)
fi
