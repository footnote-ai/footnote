...
const candidateSystemPrompt = (input: {
    ...
    input.promptVariant === 'compact'
        ? `Write only answer prose from the authoritative context. Preserve facts, evidence, uncertainty, attribution, scope, permissions, refusals, provenance, TRACE, and safety decisions. Express the resolved persona through voice, cadence, structure, emphasis, humor, bluntness, and warmth without inventing or altering boundaries. TRACE caution: ${input.caution === undefined ? 'unavailable' : input.caution}. ${input.expressionGuidance}\n\nPresentation guidance:\n${input.presentationGuidance}`
