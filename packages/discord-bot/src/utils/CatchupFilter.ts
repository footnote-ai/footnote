...
const GREETING_WHITELIST = new Set(['hey', 'yo', 'hi', 'hello']);
...
export class CatchupFilter {
    ...
    public async shouldSkipPlanner(
        ...
        try {
            ...
            const evaluationWindow = combined.slice(
                -this.RECENT_MESSAGE_WINDOW
            );

            // Presence of the bot in the recent history short-circuits several heuristics.
            const mentionDetected =
                !!botId && this.containsBotMention(evaluationWindow, botId);

            // In shared channels we only interject if someone has reached out recently.
            if (!isDirectMessage && botId && !mentionDetected) {
                logger.debug(
                    `Catchup filter heuristic (no-mention) triggered for ${channelKey}`
                );
                return {
                    skip: true,
                    reason: 'Bot not mentioned or addressed in recent context',
                };
            }

            const shouldConsiderEmojiHeuristic =
                isDirectMessage || !mentionDetected;
            if (
                shouldConsiderEmojiHeuristic &&
                this.hasEmojiOnlyStreak(evaluationWindow)
            ) {
                logger.debug(
                    `Catchup filter heuristic (emoji-only) triggered for ${channelKey}`
                );
                return {
                    skip: true,
                    reason: 'Recent messages are emoji-only or non-substantive',
                };
            }

            if (this.detectConversationPattern(evaluationWindow, botId)) {
                logger.debug(
                    `Catchup filter heuristic (other-users conversation) triggered for ${channelKey}`
                );
                return {
                    skip: true,
                    reason: 'Detected conversation between other users',
                };
            }

            const velocity = this.calculateMessageVelocity(evaluationWindow);
            const velocityThreshold =
                this.HIGH_VELOCITY_THRESHOLD /
                (this.HIGH_VELOCITY_WINDOW_MS / 1000);
            if (velocity >= velocityThreshold) {
                logger.debug(
                    `Catchup filter heuristic (high-velocity ${velocity.toFixed(2)} msg/s) triggered for ${channelKey}`
                );
            ...
            const relevanceScore = Math.max(
                ...evaluationWindow.map((msg) =>
                    this.calculateRelevanceScore(msg.content ?? '')
                ),
                0
            );
            if (relevanceScore < this.MIN_RELEVANCE_SCORE) {
                logger.debug(
                    `Catchup filter heuristic (low relevance ${relevanceScore.toFixed(2)}) triggered for ${channelKey}`
                );
            ...
            return {
                skip: false,
                reason: 'Content appears relevant for planner',
            };
        } catch (error) {
            // Fail open to keep behaviour predictable if a heuristic throws.
            logger.error(
                `Catchup filter failed for ${channelKey}: ${(error as Error)?.message ?? error}`
            );
            return {
                skip: false,
    ...
    /**
     * Scans the supplied messages for explicit mentions or replies to the bot.
     */
    private containsBotMention(messages: Message[], botId: string): boolean {
    ...
    private detectConversationPattern(
        ...
        for (const msg of nonBotMessages) {
            ...
            participantCounts.set(
                msg.author.id,
                (participantCounts.get(msg.author.id) ?? 0) + 1
            );
        }
        ...
        const alternationRatio =
            alternations / Math.max(nonBotMessages.length - 1, 1);
        ...
        const newest =
            nonBotMessages[nonBotMessages.length - 1].createdTimestamp;
    ...
    private calculateMessageVelocity(messages: Message[]): number {
        ...
        const sorted = [...humanMessages].sort(
            (a, b) => a.createdTimestamp - b.createdTimestamp
        );
        ...
        const durationMs =
            window[window.length - 1].createdTimestamp -
            window[0].createdTimestamp;
        if (durationMs <= 0) {
    ...
    private calculateRelevanceScore(content: string): number {
        ...
        if (
            /\b(tell|show|explain|help|can|could|should|would|please|anyone|someone)\b/.test(
                lower
            )
        ) {
            score += 0.3;
        ...
        // Treat friendly openings as mild signals to respond. Plaintext aliases
        // are routing candidates, not catch-up proof of assistant invocation.
