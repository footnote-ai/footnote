...
export class ResponseHandler {
    ...
    public async sendMessage(
        ...
        try {
            ...
            const hasComponents =
                ...
                (embedComponents?.length ?? 0) > 0;
            const hasFiles = files && files.length > 0;

            if (
                chunks.length === 0 &&
                (hasEmbeds || hasComponents || hasFiles)
            ) {
    ...
    public async sendEmbed(
        ...
        try {
            discordEmbed =
                embed instanceof CustomEmbedBuilder
        ...
        try {
            ...
            for (const char of emoji) {
                if (reactionCount >= MAX_REACTIONS) {
                    logger.debug(
                        `Reached maximum of ${MAX_REACTIONS} reactions`
                    );
                    break;
                }
                ...
                if (isEmoji(char)) {
                    try {
                        ...
                        logger.warn(
                            `Failed to react with emoji ${char}:`,
                            error
                        );
                    }
                }
    ...
    public async startTyping(): Promise<void> {
        ...
        const textChannel = this.channel as Extract<
            TextBasedChannel,
            { sendTyping: unknown }
        >;
    ...
    public setPresence({
        ...
        try {
            ...
            logger.info(
                `Presence updated: ${status} with ${activities.length} activities`
            );
        } catch (error) {
     ...
     */
    private applyOutboundFiltersToOptions<
            ...
            if (isCodeBlock) {
                ...
                if (part.length > maxLength) {
                    ...
                    for (const line of codeLines) {
                        ...
                        if (
                            codeChunk.length + line.length + 1 >
                            maxLength - 3
                        ) {
                            // -3 for the ```
                    ...
                    if (codeChunk.length > language.length + 5) {
                        // 5 = ``` + \n + \n
