...
export class EventManager {
    ...
    async loadEvents(eventsPath: string): Promise<void> {
        try {
            ...
            logger.debug(
                `Found ${eventFiles.length} event files in ${eventsPath}`
            );
                ...
                try {
                    ...
                    if (
                        ...
                        try {
                            ...
                            instantiated = true;
                            logger.debug(
                                `Successfully loaded event (deps ctor): ${file}`
                            );
                        } catch {
                        ...
                        if (!instantiated) {
                            try {
                                ...
                                instantiated = true;
                                logger.debug(
                                    `Successfully loaded event (client ctor): ${file}`
                                );
                            } catch {
                                // Ignore errors during event instantiation
                            }
                        ...
                        if (!instantiated) {
                            try {
                                ...
                                instantiated = true;
                                logger.debug(
                                    `Successfully loaded event (no-arg ctor): ${file}`
                                );
                            } catch (error) {
                                logger.error(
                                    `Failed to instantiate event from ${file}:`,
                                    error
                                );
                            }
                        }
                        ...
                        logger.warn(
                            `Skipping ${file}: No valid event class found`
                        );
                    }
                } catch (error) {
    ...
    public registerAll(): void {
        ...
        for (const event of this.events) {
            ...
            if (registeredEvents.has(event.name)) {
                logger.warn(
                    `Skipping duplicate registration for event: ${event.name}`
                );
                continue;
            }
