...
const renderSummary=()=>{
 const summary=add('section');summary.setAttribute('data-role','comparison-summary');summary.append(add('h2','Summary'));
...
const render=()=>{
 ...
 app.append(controls);if(blind)app.append(add('p','Blind view: model, provider, settings, cost, timing, automatic review, and saved human reviews are hidden. The source remains visible.','blind-warning'));const details=add('section');details.setAttribute('data-role',blind?'blind-review':'operational-details');details.append(add('h2',blind?'Blind review queue':'Run details'));app.append(details);
 ...
 const review=add('form');review.setAttribute('data-role','human-review');review.append(add('h3','Human review'));for(const dimension of report.config.review.rate){const label=add('label',dimension+': ');const select=document.createElement('select');const unrated=add('option','Not rated');unrated.value='';select.append(unrated);for(let score=1;score<=5;score++)select.append(add('option',String(score)));const saved=report.humanReviews.find((entry)=>entry.attemptId===attempt.attemptId&&entry.dimension===dimension);if(typeof saved?.score==='number')select.value=String(saved.score);select.onchange=()=>{report.humanReviews=report.humanReviews.filter((entry)=>!(entry.attemptId===attempt.attemptId&&entry.dimension===dimension));if(select.value)report.humanReviews.push({attemptId:attempt.attemptId,dimension,score:Number(select.value),reviewerName:report.reviewerName||undefined,at:new Date().toISOString()});save()};label.append(select);review.append(label)}
   for(const requirement of reviewRequirements){const line=add('div','');line.append(add('label',requirement.id+': '));const result=document.createElement('select');for(const value of ['Not judged','pass','fail','uncertain'])result.append(add('option',value));const saved=report.humanReviews.find((entry)=>entry.attemptId===attempt.attemptId&&entry.requirementId===requirement.id);if(typeof saved?.result==='string')result.value=saved.result;const evidence=document.createElement('textarea');evidence.placeholder='Evidence or note';evidence.value=typeof saved?.evidence==='string'?saved.evidence:'';const update=()=>{report.humanReviews=report.humanReviews.filter((entry)=>!(entry.attemptId===attempt.attemptId&&entry.requirementId===requirement.id));if(result.value!=='Not judged'||evidence.value){report.humanReviews.push({attemptId:attempt.attemptId,requirementId:requirement.id,result:result.value,evidence:evidence.value,reviewerName:report.reviewerName||undefined,at:new Date().toISOString()})}save()};result.onchange=update;evidence.oninput=update;line.append(result,evidence);review.append(line)}card.append(review)
  }
   details.append(card)
 }
};render();
