...
const renderSummary=()=>{
 ...
 summary.append(add('p','Must-keep categories: '+report.config.review.mustKeep.join(', ')+' (automatic and human evidence stay separate; no combined winner score)','muted'));
 if(!blind){
  summary.append(add('h3','Comparison evidence'));
  ...
  const table=document.createElement('table');table.className='comparison';const head=document.createElement('tr');for(const label of ['Model / setting','Ran','Automatic review coverage','Automatic must-keep','Automatic ratings','Human review','Typical latency','Generation cost','Review cost','Total cost','Median output tokens / total second'])head.append(add('th',label));table.append(head);
  ...
  summary.append(table);summary.append(add('p','Automatic review columns use only completed automatic reviews; human review columns use only saved human judgments. Coverage is shown separately. Click a model/setting cell to inspect its attempts.','muted'));
  ...
  summary.append(add('h3','Automatic must-keep findings'));summary.append(add('p',findings.length?findings.join(' | '):'No automatic must-keep failures, uncertainties, or review failures recorded.','muted'));
 }
 return summary;
};
const render=()=>{
 ...
 app.append(controls);if(blind)app.append(add('p','Blind view: model, provider, settings, cost, latency, automatic review, and saved human reviews are hidden. Source context remains visible.','blind-warning'));app.append(add('h2',blind?'Blind review queue':'Response provenance'));
  ...
  if(!blind){card.append(add('p','Model: '+JSON.stringify(attempt.model)));card.append(add('p','Setting: '+JSON.stringify(attempt.setting)));card.append(add('p','Support evidence: '+JSON.stringify(attempt.support||{})));card.append(add('p','Operations: '+JSON.stringify(attempt.operations||{})));card.append(add('p','Attribution: '+JSON.stringify(attempt.attribution||{})));if(attempt.automaticReview)card.append(add('pre','Automatic review instruction/schema and result:\n'+JSON.stringify(attempt.automaticReview,null,2)))}
